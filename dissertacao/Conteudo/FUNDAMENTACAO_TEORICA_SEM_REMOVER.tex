...
Diversas técnicas foram desenvolvidas para representar palavras vetorialmente (\textit{word embeddings}), diferenciando-se por suas arquiteturas e bases de dados usadas no treinamento \cite{martin2009}.
...
\begin{itemize} 
   ...
   \begin{itemize}
    ...
    \item O \textbf{coeficiente de correlação de Pearson}: Mede a relação estatística entre duas variáveis contínuas, onde esse valor varia entre -1 (relação negativa) e 1 (relação positiva). O valor 0 indica que não há relação linear. A Equação \ref{eq:3.3.5} representa a correlação de Pearson, onde \textit{x} é a variável número 1, \textit{y} é a variável número 2, \textit{$z_{x}$} é o desvio padrão da variável 1, \textit{$z_{y}$} é o desvio padrão da variável número 2, e \textit{N} é a quantidade de dados. A vantagem de utilizar essa medida é que o valor é independente de qualquer unidade usada para medir as variáveis, e, se a amostra for grande, a precisão da estimativa é maior \cite{mukaka2012}.
    ...
    \item O \textbf{coeficiente de correlação de Spearman}: Mede a força e a direção da associação entre duas variáveis, considerando as previsões e os valores verdadeiros como classificações, e não seus valores reais. É utilizado quando existe uma relação monótona, ou seja, quando a função nunca diminui ou nunca aumenta. O coeficiente de Spearman pode ter um valor entre -1 (associação negativa perfeita) e 1 (associação de classificação perfeita), enquanto o valor 0 indica que não há associação de classificação. A Equação \ref{eq:3.3.6} representa a correlação de Spearman, onde \textit{n} é a quantidade de pontos das duas variáveis e \textit{$d_{i}$} é a diferença de alcance do \textit{n} \cite{mukaka2012}. A correlação de Spearman corresponde à correlação de Pearson aplicada às classificações.
...
Além disso, coeficientes como os de correlação de Pearson e Spearman são utilizados para avaliar relações entre variáveis contínuas e ordinais, respectivamente. Essas métricas são especialmente úteis para analisar a força e direção da relação entre variáveis.

\begin{itemize}    
    \item O \textbf {coeficiente de correlação de Pearson}: Mede a relação estatística entre duas variáveis continuas, onde esse valor varia entre -1 (relação negativa) e 1 (relação positiva) e o 0 indica que não tem nenhuma relação linear. A Equação \ref{eq:3.3.7} representa a correlação de Pearson onde \textit{x} é à variável número 1, \textit{y} é a variável número 2, \textit{$z_{x}$} é o desvio padrão da variável 1, \textit{$z_{y}$} é o desvio padrão da variável número 2 e \textit{N} é a quantidade de dados. A vantagem de utilizar essa medida é que o valor é independente de qualquer unidade usada para medir as variáveis e se a amostra for grande, a precisão da estimativa da amostra é mais provável \cite{mukaka2012}.
    ...
    \item O \textbf {coeficiente de correlação de Spearman}: Mede a força e direção da associação entre duas variáveis e ela considera as previsões e os valores verdadeiros como classificação, e não o seu valor real. Ela é utilizada quando possui uma relação monótona, ou seja,  quando a função nunca diminui ou nunca aumenta. O coeficiente de Spearman pode ter um valor entre -1 (associação negativa perfeita) e 1 (associação de classificação perfeita) e o valor 0 indica que não há associação de classificação. A Equação \ref{eq:3.3.8} é representada pela correlação de Spearman, onde o \textit{n} é a quantidade de pontos das duas variáveis, o \textit{$d_{i}$} é a diferença de alcance do \textit{n} \cite{mukaka2012}. A correlação de Spearman corresponde a correlação de Pearson aplicado a classificação.
...
Contudo, quando utilizado para classificar padrões, uma rede perceptron de três camadas é limitada a encontrar uma única solução. Para resolver problemas que exigem múltiplas soluções, Minsky e Papert (1969) propuseram a inclusão de múltiplas camadas intermediárias, resultando no modelo conhecido como perceptron multicamadas (Multi-layer Perceptron – MLP ou Multi-layer Neural Network – MLNN) \cite{minsky1969}.
...
\textit{W} representando os pesos de cada neurônio em uma camada, essas redes tendem a se tornar profundas. Essa profundidade pode gerar problemas como a dissipação do gradiente (gradient vanishing) e a explosão do gradiente (gradient explosion).
...
Para mitigar esses problemas, os cientistas Hochreiter e Schmidhuber (1997) propuseram o LSTM (Long Short-Term Memory ou Memória de Longo e Curto Prazo). Nesse modelo, os neurônios tradicionais foram substituídos por células de memória, permitindo que as redes RNN superem as limitações associadas à dissipação e à explosão do gradiente \cite{hochreiter1997}.
...
Com o desenvolvimento do ChatGPT e do GPT-4 (Generative Pre-trained Transformer) pela OpenAI, ocorreu uma revolução na percepção sobre os LLMs \cite{openai2024}. Apesar de possuírem desempenho notável, a OpenAI não divulgou detalhes sobre as estratégias de treinamento ou os parâmetros de peso utilizados.
...
\begin{itemize}
    \item \textbf{Pesquisa vetorial}: Utiliza representações numéricas de alta dimensão para codificar o conteúdo semântico dos textos.
    \item \textbf{Similaridade vetorial}: Aplica métricas matemáticas para quantificar a proximidade semântica entre vetores. As métricas mais comuns da similaridade vetorial são: Similaridade do Cosseno, Distância Euclidiana e Produto Escalar.
...
A metodologia Tx3 propõe três ações a serem realizadas conforme a classificação das informações presentes nos regulamentos: (1) Transcrever (T1): Deve ser aplicada para instruções que podem ser diretamente transcritas em regras computáveis; (2) Transformar (T2): Deve ser aplicada para instruções que necessitam ser reestruturadas ou reescritas de forma a preservar o sentido original; (3) Transferir (T3): 

Deve ser aplicada quando os requisitos são expressos de maneira imprecisa, sem conexão clara entre os objetivos do regulamento e os indicadores. Nestes casos, é necessária a análise de um profissional especializado. Após a classificação do texto original, os elementos classificados como T1 e T2 são reescritos e submetidos à metodologia RASE.

A metodologia RASE, baseada em conceitos semânticos, transforma documentos normativos em regras bem definidas, utilizáveis em softwares baseados em BIM \cite{hjelseth2011}. O processo consiste na identificação de quatro operadores lógicos:
...
\begin{itemize}
    ...
    \item \textbf{Unidade}: Tipo de unidade do valor numérico se existir. 
\end{itemize}
