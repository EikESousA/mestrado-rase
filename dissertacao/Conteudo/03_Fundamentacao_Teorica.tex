...
As normas técnicas de engenharia constituem o arcabouço regulatório que define requisitos mínimos de segurança, desempenho, qualidade e interoperabilidade nos projetos do setor de Arquitetura, Engenharia e Construção (AEC). No Brasil, a regulamentação é coordenada pela Associação Brasileira de Normas Técnicas (ABNT), responsável pela elaboração das Normas Brasileiras (NBR). Um exemplo recorrente é a NBR 9050, que estabelece os critérios de acessibilidade a edificações, mobiliário, espaços e equipamentos urbanos \cite{abnt9050}. No plano internacional, a Organização Internacional de Padronização (ISO) publica normas adotadas por diversos países, sendo a série ISO 19650 (Organização e digitalização da informação sobre obras de construção, incluindo a Modelagem da Informação da Construção) a referência consolidada para a gestão da informação em BIM \cite{iso19650}.

Apesar do papel essencial das normas, sua aplicação ainda é predominantemente manual: o profissional precisa interpretar trechos extensos de texto, identificar os requisitos pertinentes e verificá-los, item a item, no projeto. Esse esforço cresce de forma combinatória em projetos BIM, que podem conter centenas ou milhares de elementos sujeitos a múltiplas normas simultaneamente. Eastman e colaboradores observam que a verificação manual de conformidade é onerosa, propensa a erros e dificilmente escalável, motivando a pesquisa em mecanismos automáticos de checagem baseados em regras \cite{eastman2009}.

A automação dessa verificação esbarra, contudo, em três obstáculos. Em primeiro lugar, os textos normativos são escritos em linguagem natural, frequentemente com ambiguidades e referências cruzadas que dificultam a sua tradução direta para regras computáveis \cite{dimyadi2013}. Em segundo, as normas variam entre regiões e contextos, o que limita o reaproveitamento de regras já formalizadas em outros países \cite{vargas2023}. Em terceiro, a classificação semântica dos itens normativos (o que é um requisito, o que é uma aplicabilidade, o que é uma exceção) é não trivial e requer um esquema padronizado que possa ser implementado por diferentes ferramentas \cite{solihin2015}.

Para enfrentar essas dificuldades, abordagens semântico-formais propõem a estruturação prévia das normas em um esquema computável, antes da execução da verificação propriamente dita. Beach e colaboradores demonstram que um sistema baseado em regras pode automatizar a checagem regulatória no setor da construção quando as normas estão devidamente formalizadas \cite{beach2015}. Nesse contexto, a metodologia RASE, introduzida por Hjelseth \cite{hjelseth2011}, oferece uma forma sistemática de marcar os trechos normativos como Requisito, Aplicabilidade, Seleção ou Exceção, criando um substrato adequado para verificação automatizada em modelos BIM. Em paralelo, abordagens recentes baseadas em Modelos de Linguagem de Grande Porte vêm sendo investigadas como caminho para reduzir o custo de tradução das normas em representações computáveis \cite{fuchs2024,du2024}.
...
A avaliação de saídas geradas por LLMs em tarefas de conversão de texto requer a comparação entre o texto produzido pelo modelo e um texto de referência. Como nem sempre a correspondência exata é desejável --- muitas vezes paráfrases corretas devem ser aceitas ---, são utilizadas métricas de similaridade que se dividem, de forma geral, em quatro famílias: \textit{lexicais}, \textit{semânticas}, \textit{de distância} e \textit{voltadas para geração} \cite{jurafsky2009}.

As métricas \textbf{lexicais} comparam as cadeias de caracteres ou os tokens presentes nos textos, sem considerar o significado das palavras. Uma das mais difundidas é a baseada na distância de Levenshtein, que conta o número mínimo de operações de edição (inserção, remoção ou substituição) necessárias para transformar uma cadeia em outra. A biblioteca \textit{FuzzyWuzzy} oferece variantes dessa medida, como o \textit{partial ratio}, adequado para casos em que uma string é fragmento da outra. Outra métrica lexical clássica é a similaridade do cosseno aplicada a vetores TF-IDF (\textit{Term Frequency--Inverse Document Frequency}), que pondera os termos pela sua importância relativa no corpus \cite{manning2008}.

As métricas \textbf{semânticas} apoiam-se em \textit{sentence embeddings}: representam cada texto como um vetor em um espaço de alta dimensionalidade, treinado para aproximar sentenças com significado parecido. A similaridade é então calculada pelo cosseno entre os vetores. Modelos como o \textit{Sentence-BERT} (SBERT) \cite{reimers2019} estendem o BERT para gerar tais representações de forma eficiente. Para o português, destaca-se o BERTimbau \cite{souza2020}, especializado em textos em português brasileiro, com variantes ajustadas para o domínio jurídico. Modelos multilíngues, como o \textit{paraphrase-multilingual-mpnet-base}, permitem comparações entre línguas diferentes em um mesmo espaço vetorial.

As métricas \textbf{de distância} medem o custo de transformar um texto em outro no espaço de \textit{embeddings}. O \textit{Word Mover's Distance} (WMD) \cite{kusner2015} formaliza esse problema como um transporte ótimo entre as palavras de duas sentenças, ponderado pela distância euclidiana entre seus \textit{embeddings}. Para o português, o WMD pode ser calculado com \textit{embeddings} do FastText \cite{bojanowski2017} ou com modelos do projeto NILC, treinados em corpora em português. Valores normalizados de WMD aproximam-se de 1 quando as sentenças são semanticamente próximas e tendem a 0 quando distantes.

As métricas \textbf{voltadas para geração} surgiram da literatura de tradução automática e sumarização para avaliar saídas em que a forma exata da resposta pode variar. O \textit{BERTScore} \cite{zhang2020bertscore} alinha contextualmente cada \textit{token} da saída ao \textit{token} mais próximo da referência, com base em \textit{embeddings} do BERT, agregando o resultado em medidas de precisão, revocação e F1 contextuais. O \textit{ROUGE-L} \cite{lin2004rouge}, por sua vez, mede a razão entre a maior subsequência comum (\textit{Longest Common Subsequence}) e o comprimento da referência, captando paralelismo sintático mesmo quando há inserções intermediárias.

Essas quatro famílias são complementares: as lexicais e o ROUGE-L penalizam reformulações que preservam o significado, enquanto as semânticas, as de distância e o BERTScore capturam equivalências apesar de variações de superfície. A combinação das quatro fornece um retrato mais completo da qualidade das saídas de LLMs e é amplamente utilizada na literatura de PLN aplicada \cite{reimers2019}.
...
Dentre as técnicas para especializar LLMs em tarefas de domínio específico sem alterar seus pesos, destaca-se a \textit{Engenharia de Prompt}, que envolve a preparação cuidadosa das instruções enviadas ao modelo para obter respostas características, concisas e contextualmente adequadas. A Engenharia de Prompt tornou-se uma prática essencial para o aproveitamento máximo das potencialidades dos LLMs \cite{brown2020}, sendo a abordagem adotada nesta pesquisa.
