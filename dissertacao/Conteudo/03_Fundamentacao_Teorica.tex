...
\item \textbf{BIM 3D --- Modelo Virtual 3D}: estende a modelagem 2D para uma representação completa em X, Y e Z. Algoritmos vindos da computação gráfica detectam colisões antes da obra começar --- e essa detecção precoce derruba o custo de retrabalho no canteiro.
...
\subsubsection*{NBR~9050 --- Acessibilidade}
...
Em classificação, o modelo deve generalizar os padrões aprendidos no treino para categorizar instâncias novas. Em regressão, o alvo é um valor contínuo --- prever a venda futura de um produto, estimar o preço de um imóvel a partir de variáveis históricas, calcular a expectativa de vida de um país \cite{guimaraes2019}.
...
O pré-processamento ataca os ruídos que aparecem nos dados brutos --- anomalias, valores ausentes, atributos irrelevantes que prejudicam o aprendizado. O sucesso dessa etapa depende, em boa medida, da habilidade do profissional em diagnosticar o problema e escolher a técnica certa \cite{neves2003}.
...
Em 1950, Alan Turing --- frequentemente lembrado como o pai da computação --- propôs o Teste de Turing: um experimento em que um avaliador tenta decidir se está conversando com uma pessoa ou com uma máquina. É comum apontar esse teste como o marco fundador da reflexão sobre Processamento de Linguagem Natural (PLN) \cite{russel2010}.
...
Implementar todos os níveis em um único sistema é raro: a complexidade cresce a cada camada. Mesmo assim, o PLN aparece em uma quantidade enorme de aplicações --- reconhecedores e sintetizadores de fala, corretores ortográficos, tradutores automáticos, geração e sumarização de texto, extração de informações, interfaces de linguagem natural \cite{vieira2001}.
...
\begin{itemize}
    ...
    \item \textbf{Relações semânticas}: em um espaço n-dimensional, palavras com significado próximo ficam vizinhas --- ``macaco'' e ``gorila'', por exemplo.
...
O LSTM é uma evolução da RNN. Seu funcionamento se organiza em cadeia: cada camada representa um intervalo de tempo $t$, e a rede contém células de memória --- cada célula com três portões (entrada, esquecimento e saída).
...
Um bom \textit{prompt} é claro e preciso: dá ao modelo o que ele precisa para compreender a tarefa e indica o formato esperado da resposta. Para a família LLaMA, Zhang argumenta que \textit{prompts} curtos e simples funcionam melhor --- o modelo entende a tarefa com mais facilidade \cite{zhang2024}.
...
A integração do BIM com tecnologias de IA generativa (IAG) --- ChatGPT e congêneres --- ainda é pouco explorada no setor AEC. O tema cobre aplicações práticas, \textit{frameworks} propostos, desafios em aberto e direções futuras, sendo objeto de uma onda recente de trabalhos.
